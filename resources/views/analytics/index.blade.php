    ...
    <div class="space-y-8">
        <section class="rounded-[2rem] border border-slate-200/70 bg-gradient-to-br from-white via-white to-[#146EDB]/5 p-6 shadow-sm sm:p-8">
            <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-3xl">
                    <div class="inline-flex items-center rounded-full bg-[#146EDB]/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#146EDB]">
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <article class="rounded-2xl border border-emerald-200/70 bg-white p-5 shadow-sm">
                ...
                <div class="mt-1 text-sm text-slate-600">Órdenes ya entregadas</div>
            </article>

            <article class="rounded-2xl border border-amber-200/70 bg-white p-5 shadow-sm">
            ...
            <div class="rounded-[2rem] border border-slate-200/70 bg-white p-6 shadow-sm">
                ...
                <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    @foreach ($statusLabels as $status => $label)
                        <a href="{{ route('orders.index', ['status' => $status]) }}" class="rounded-2xl border border-slate-200/70 bg-slate-50/70 p-4 transition hover:-translate-y-0.5 hover:border-[#146EDB]/25 hover:bg-white hover:shadow-sm">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1 {{ $statusBadgeClasses[$status] ?? 'bg-slate-100 text-slate-700 ring-slate-200/70' }}">
                ...
                <div class="mt-6 space-y-4">
                    <div class="rounded-2xl bg-slate-50 p-4">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <div class="rounded-2xl bg-[#146EDB]/10 px-3 py-2 text-[#146EDB]">#</div>
                        </div>
                    </div>
                        ...
                        <article class="rounded-2xl border border-slate-200/70 bg-slate-50/70 p-4">
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <div class="flex shrink-0 flex-col items-end gap-1">
                                    <span class="rounded-full bg-[#146EDB]/10 px-3 py-1 text-xs font-semibold text-[#146EDB]">
        ...
        <section class="rounded-[2rem] border border-slate-200/70 bg-white p-6 shadow-sm">
            ...
            <div class="mt-5 space-y-3">
                @forelse ($recentOrders as $order)
                    <article class="rounded-2xl border border-slate-200/70 bg-slate-50/70 p-4 transition hover:border-[#146EDB]/25 hover:bg-white">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="text-sm font-semibold text-brand-navy">#{{ $order->id }}</span>
                                    <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1 {{ $statusBadgeClasses[$order->status] ?? 'bg-slate-100 text-slate-700 ring-slate-200/70' }}">
                            ...
                            <a href="{{ route('orders.show', $order) }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-[#146EDB]/25 hover:text-[#146EDB]">
                                Abrir pedido
                            </a>
                        </div>
                    </article>
