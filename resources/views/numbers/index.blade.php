<x-app-layout>
    ...
    <div
        ...
        x-data="{
            ...
            selectedLabel: @js(old('number') ? 'Pedido manual para el código ' . old('number') : 'Selecciona un código'),
            allNumbers: @js($allNumbers),
            openNumber(number) {
                this.selectedNumber = number;
                this.selectedLabel = `Pedido manual para el código ${number}`;
                this.modalOpen = true;
            },
    ...
    >
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            ...
            <a href="{{ route('intake-requests.index') }}" class="brand-btn-secondary">Volver a pedidos</a>
        </div>

        @if (session('status'))
            <div class="rounded-2xl border border-brand-amber/20 bg-amber-50 px-4 py-3 text-sm text-amber-900 shadow-sm">
                {{ session('status') }}
            </div>
        @endif
        ...
        <div class="brand-card p-5 sm:p-6">
            <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                <div>
                    <div class="text-sm font-medium text-slate-700">Sucursal</div>
                        ...
                        <div>
                            ...
                            @else
                                ...
                                <div class="rounded-xl border border-slate-200/80 bg-slate-50 px-3 py-2.5 text-sm text-slate-700">
                                    {{ $selectedBranch?->name ?? 'Sin sucursal disponible' }}
                                </div>
                            @endif
                        </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="rounded-2xl border border-slate-200/80 bg-slate-50 p-4">
                        <div class="text-sm text-slate-500">Sucursal seleccionada</div>
                        <div class="mt-1 text-base font-semibold text-brand-navy">{{ $selectedBranch?->name ?? 'Sin sucursal disponible' }}</div>
                    </div>
                    <div class="rounded-2xl border border-slate-200/80 bg-slate-50 p-4">
                        <div class="text-sm text-slate-500">Referencia seleccionada</div>
                        <div class="mt-1 text-base font-semibold text-brand-navy">{{ $selectedDraw?->name ?? 'Sin referencia disponible' }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
            <div class="brand-card border-brand-secondary/20 p-5">
                <div class="text-sm font-medium text-brand-secondary">Total confirmado</div>
            ...
            <div class="brand-card border-brand-primary/20 p-5">
                <div class="text-sm font-medium text-brand-primary">Total pendiente</div>
                <div class="mt-1 text-2xl font-semibold tracking-tight text-brand-navy">&#8353;{{ number_format($summary['pending_amount'], 2, '.', ',') }}</div>
            </div>
            <div class="brand-card border-brand-primary/20 p-5">
                <div class="text-sm font-medium text-brand-primary">En revisión</div>
                <div class="mt-1 text-2xl font-semibold tracking-tight text-brand-navy">&#8353;{{ number_format($summary['needs_review_amount'], 2, '.', ',') }}</div>
            </div>
            <div class="brand-card border-brand-amber/20 p-5">
                <div class="text-sm font-medium text-brand-amber">Total vigente</div>
            ...
            <div class="brand-card border-brand-warning/20 p-5">
                <div class="text-sm font-medium text-brand-warning">Códigos cerca del límite</div>
            ...
            <div class="brand-card border-brand-danger/20 p-5">
                <div class="text-sm font-medium text-brand-danger">Códigos sobre el límite</div>
        ...
        <div class="brand-card p-4">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="brand-badge bg-emerald-50 text-emerald-700">Disponible <span class="font-normal text-slate-500">menos del 80% o sin tope</span></span>
                    <span class="brand-badge bg-amber-50 text-amber-700">Advertencia <span class="font-normal text-slate-500">80% a 99%</span></span>
                    <span class="brand-badge bg-blue-50 text-blue-700">Completo <span class="font-normal text-slate-500">100%</span></span>
                    <span class="brand-badge bg-red-50 text-red-700">Sobre el límite <span class="font-normal text-slate-500">&gt; 100%</span></span>
                    <span class="brand-badge bg-slate-100 text-slate-700">Sin tope <span class="font-normal text-slate-500">sin límite configurado</span></span>
                </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div class="relative w-full sm:w-80">
                        <label for="number-search" class="sr-only">Buscar código</label>
                        <input
                            id="number-search"
                            type="search"
                            x-model="searchTerm"
        ...
        <section class="space-y-4">
            <div x-show="view === 'grid'" x-cloak class="grid items-start gap-4 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        >
                            <div class="flex items-center gap-3">
                                <div class="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-slate-100 text-slate-900">
                                    <span class="h-2.5 w-2.5 rounded-full {{ $groupTone === 'available' ? 'bg-brand-secondary' : ($groupTone === 'warning' ? 'bg-brand-warning' : ($groupTone === 'full' ? 'bg-brand-primary' : 'bg-brand-danger')) }}"></span>
                                </div>
                                <div>
                                    <div class="text-sm font-semibold text-brand-navy">{{ sprintf('%02d-%02d', $start, $end) }}</div>
                                    <div class="text-xs text-slate-500">Grupo {{ $groupIndex + 1 }}</div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-2 xl:grid-cols-5">
                                @foreach ($group as $row)
                                    @php
                                        ...
                                        $statusLabel = $hasLimit ? str_replace('_', ' ', $row['status']) : 'sin tope';
                                        $cardClasses = match ($row['status']) {
                                            ...
                                            default => $hasLimit ? 'border-brand-secondary/20 bg-green-50/80 text-green-700' : 'border-slate-200 bg-slate-50 text-slate-600',
                                        };
                                            ...
                                            <div class="space-y-1 text-center">
                                                ...
                                                @if ($hasLimit)
                                                    <div class="text-[11px] leading-none text-slate-500">
                                                        Disponible: &#8353;{{ number_format(max((float) $row['available_amount'], 0), 0, '.', ',') }}
                                                    </div>
                                                @endif
                                            </div>
                ...
                >
                    Ningún código coincide con la búsqueda actual.
                </div>
            </div>
                    ...
                    <table class="min-w-[1220px] divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Confirmado</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Pendiente</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">En revisión</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Rechazado</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Total vigente</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Tope máximo</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Disponible</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">% usado</th>
                            ...
                            @foreach ($numbers as $row)
                                @php
                                    ...
                                    $statusClasses = match ($row['status']) {
                                        ...
                                        default => $hasLimit ? 'border-brand-secondary/20 bg-green-100 text-green-700' : 'border-slate-200 bg-slate-200 text-slate-600',
                                    };
                                    $statusLabel = $hasLimit ? str_replace('_', ' ', $row['status']) : 'sin tope';
                                @endphp
                                <tr class="{{ $tableRowClasses }}" x-show="matchesNumber(@js($row['number']))">
                                    ...
                                    <td class="px-4 py-3 text-sm text-slate-700">{{ $row['max_amount'] !== null ? '₡' . number_format((float) $row['max_amount'], 2, '.', ',') : 'Sin tope' }}</td>
            ...
            >
                <div class="w-full max-w-2xl rounded-3xl border border-slate-200/80 bg-white shadow-[0_30px_80px_-36px_rgba(8,31,77,0.6)]">
                    <div class="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-5 sm:px-6">
                        <div>
                            <div class="brand-badge bg-brand-primary/10 text-brand-primary">Pedido manual</div>
                            <h2 class="mt-2 text-lg font-semibold text-brand-navy">Crear pedido manual</h2>
                            <p class="mt-1 text-sm text-slate-600" x-text="selectedLabel"></p>
                        </div>
                        <button type="button" class="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-900" @click="modalOpen = false" aria-label="Cerrar modal">
                    ...
                    <form method="POST" action="{{ route('numbers.store') }}" class="space-y-5 px-5 py-5 sm:px-6">
                        ...
                        <div class="grid gap-4 sm:grid-cols-3">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700">Código seleccionado</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700" for="amount">Monto</label>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <label class="block text-sm font-medium text-slate-700" for="customer_name">Nombre del cliente</label>
                                <input id="customer_name" name="customer_name" type="text" value="{{ old('customer_name') }}" class="brand-input mt-1 block w-full rounded-xl" placeholder="Opcional">
                                @error('customer_name')<p class="mt-2 text-sm text-brand-danger">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700" for="customer_phone">Teléfono del cliente</label>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea id="notes" name="notes" rows="4" class="brand-input mt-1 block w-full rounded-xl" placeholder="Opcional">{{ old('notes') }}</textarea>
