<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <section class="overflow-hidden rounded-[2rem] border border-slate-200/80 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 shadow-sm">
            <div class="grid gap-6 px-6 py-7 lg:grid-cols-[1.2fr_0.8fr] lg:px-8">
                <div class="space-y-4">
                    ...
                    <div class="max-w-3xl space-y-3">
                        <h1 class="text-3xl font-semibold tracking-tight text-brand-navy sm:text-4xl">Catalogo de productos</h1>
                        <p class="max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
                            Administra los productos que Benditio puede reconocer automaticamente.
                        </p>
                    </div>
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    <article class="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-sm">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Ultima actualizacion</p>
        ...
        @if ($productItems->isEmpty())
            <section class="rounded-[2rem] border border-dashed border-slate-300 bg-white px-6 py-16 text-center shadow-sm">
                <div class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-brand-primary/10 text-4xl">
                    📦
                </div>
                <h2 class="mt-6 text-2xl font-semibold text-brand-navy">Aun no hay productos.</h2>
                <p class="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-600">
                    Comienza creando un producto o importando tu catalogo.
                </p>
            ...
            <section class="space-y-4">
                <div class="flex items-end justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-600">Vista rapida para revisar estado, SKU y alias.</p>
                    </div>
                ...
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    @foreach ($products as $product)
                        ...
                        <article class="flex h-full flex-col rounded-[1.75rem] border border-slate-200/80 bg-white p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-lg">
                            <div class="flex items-start justify-between gap-4">
                                <div class="min-w-0">
                                    ...
                                    <div class="mt-3 flex flex-wrap gap-2">
                                        ...
                                        <span class="inline-flex rounded-full bg-blue-50 px-2.5 py-1 text-xs font-medium text-brand-primary">
                                            Unidad: {{ $product->unit_label ?? 'Sin unidad' }}
                                        </span>
                                    </div>
                                </div>
                                <span class="inline-flex shrink-0 rounded-full px-3 py-1 text-xs font-semibold {{ $product->is_active ? 'bg-emerald-50 text-green-700' : 'bg-slate-100 text-slate-600' }}">
                            ...
                            <div class="mt-5 space-y-3">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @forelse ($aliasPreview as $alias)
                                        <span class="inline-flex rounded-full bg-brand-primary/8 px-3 py-1 text-xs font-medium text-brand-navy">
                                            {{ $alias->alias }}
                                        </span>
                                    @empty
                                ...
                                @if ($aliases->count() > 3)
                                    <p class="text-xs text-slate-500">+{{ $aliases->count() - 3 }} alias mas</p>
                                @endif
                            </div>
